...
// Start snow - exported for theme.js to use
const startSnow = () => {
    snowEnabled = true;
    animateSnow();
};

// Stop snow - exported for theme.js to use
const stopSnow = () => {
    snowEnabled = false;
...
// Export functions globally for theme.js
window.startSnow = startSnow;
window.stopSnow = stopSnow;

// Auto-initialize: Check if we should start snow based on current mode
const currentMode = localStorage.getItem('mode');
if (currentMode === 'snow') {
    startSnow();
} else {
    // During Christmas period with no saved mode, default to snow
    const now = new Date();
    const month = now.getMonth();
    const day = now.getDate();
    const isChristmas = (month === 11) || (month === 0 && day <= 6);

    if (!currentMode && isChristmas) {
        startSnow();
    }
}
